...
Role \textbf{user} example result, here user dh3wr is asking:
\begin{lstlisting}
{
  "total_rows": 2,
  "offset": 0,
...
Mapping to CouchDB with filtering on microservice:\\
...
Role \textbf{user} example result (dh3wr is asking for dl2ic):
\begin{lstlisting}
{
  "_id": "dl2ic",
  "role": "admin",
...
\subsubsection{PUT /users/<username> - Add new user}
...
\subsubsection{PUT /users/<username> - Edit existing user}
...
Role \textbf{admin} or \textbf{support} or deleting the requestor's own entry are the only allowed roles.
